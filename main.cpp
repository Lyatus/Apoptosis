        ...
        for(int x(0); x<Chunk::size; x++)
          for(int y(0); y<Chunk::size; y++)
            for(int z(0); z<Chunk::size; z++) {
              ...
              if(budPotential && voxel.type()==Voxel::TUMOR_IDLE && position.y()>buddingMinY && !Automaton::has(growth,(int)buddingCount) && Rand::nextFloat()<buddingFactor*(buddingCurve/(Bonus::distanceToInactive(position)+buddingCurve)) && !Automaton::has(growth,position))
